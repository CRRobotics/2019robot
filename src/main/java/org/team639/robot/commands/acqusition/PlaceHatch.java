...
public class PlaceHatch extends CommandGroup {
    public PlaceHatch() {
        addSequential(new ConditionalCommand(new ExecuteCommandAndWait(new ExtendFlower(), HATCH_EXTEND_DELAY)) {
            @Override
            protected boolean condition() {
                return !Robot.acquisition.isFlowerExtended();
            }
        });
        addSequential(new ExecuteCommandAndWait(new CloseFlower(), HATCH_OPEN_DELAY));
        addSequential(new RetractFlower());
    }
}
